...
find ${TEST_DIR} -iname "*.h" | xargs clang-format -i -style=file
find ${SRC_DIR} -iname "*.cc" | xargs clang-format -i -style=file
find ${TEST_DIR} -iname "*.cc" | xargs clang-format -i -style=file
find ${DRIVERS_DIR} -iname "*.cc" | xargs clang-format -i -style=file
find ${INCLUDE_DIR} -iname "*.h" | xargs clang-format -i -style=file
find ${TEST_DIR} -iname "*.h" | xargs clang-format -i -style=file
