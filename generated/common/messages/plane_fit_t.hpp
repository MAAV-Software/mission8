...
uint32_t plane_fit_t::getEncodedSize() const { return 8 + _getEncodedSizeNoHash(); }
...
const char* plane_fit_t::getTypeName() { return "plane_fit_t"; }
...
uint64_t plane_fit_t::_computeHash(const __zcm_hash_ptr* p)
{
    ...
    for (fp = p; fp != NULL; fp = fp->parent)
        if (fp->v == plane_fit_t::getHash) return 0;
    const __zcm_hash_ptr cp = {p, (void*)plane_fit_t::getHash};

    uint64_t hash = (uint64_t)0x76301ca66853bc93LL + vector1_t::_computeHash(&cp) +
                    vector1_t::_computeHash(&cp) + vector1_t::_computeHash(&cp) +
