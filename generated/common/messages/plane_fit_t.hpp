...
int plane_fit_t::_encodeNoHash(void* buf, uint32_t offset, uint32_t maxlen) const
{
    ...
    thislen = __float_encode_array(buf, offset + pos, maxlen - pos, &this->z_dot, 1);
    if(thislen < 0) return thislen; else pos += thislen;
...
int plane_fit_t::_decodeNoHash(const void* buf, uint32_t offset, uint32_t maxlen)
{
    ...
    thislen = __float_decode_array(buf, offset + pos, maxlen - pos, &this->z_dot, 1);
    if(thislen < 0) return thislen; else pos += thislen;
...
{
    uint64_t hash = (uint64_t)0xe14b880c14a71c46LL;
