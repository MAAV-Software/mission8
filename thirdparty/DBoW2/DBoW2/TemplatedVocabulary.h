...
class TemplatedVocabulary {
   public:
     ...
     */
    virtual void create(
        const std::vector<std::vector<TDescriptor> >& training_features, int k,
        int L);
   ...
   protected:
     ...
     */
    void getFeatures(const std::vector<vector<TDescriptor> >& training_features,
     ...
     * @param levelsup
     */
    virtual void transform(const TDescriptor& feature, WordId& id,
                           WordValue& weight, NodeId* nid = NULL,
    ...
    /**
     * Creates k clusters from the given descriptors with some seeding
     * algorithm.
     ...
     * @param features
     */
    void setNodeWeights(const std::vector<vector<TDescriptor> >& features);
...
void TemplatedVocabulary<TDescriptor, F>::create(
    ...
    m_nodes.reserve(
        expected_nodes);  // avoid allocations when creating the tree
...
void TemplatedVocabulary<TDescriptor, F>::getFeatures(
    const std::vector<vector<TDescriptor> >& training_features,
    std::vector<pDescriptor>& features) const {
    features.resize(0);

    typename std::vector<vector<TDescriptor> >::const_iterator vvit;
    typename std::vector<TDescriptor>::const_iterator vit;
    for (vvit = training_features.begin(); vvit != training_features.end();
         ++vvit) {
...
void TemplatedVocabulary<TDescriptor, F>::HKmeansStep(
    NodeId parent_id, const std::vector<pDescriptor>& descriptors,
    int current_level) {
    if (descriptors.empty()) return;

    // features associated to each cluster
    std::vector<TDescriptor> clusters;
    std::vector<vector<unsigned int> > groups;  // groups[i] = [j1, j2, ...]
    ...
    if ((int)descriptors.size() <= m_k) {
        ...
        while (goon) {
            ...
            if (first_time) {
                ...
                for (unsigned int c = 0; c < clusters.size(); ++c) {
                    ...
                    */
                    ...
                    for (vit = groups[c].begin(); vit != groups[c].end();
                         ++vit) {
            ...
            // unsigned int d = 0;
            for (fit = descriptors.begin(); fit != descriptors.end();
                 ++fit)  //, ++d)
            {
            ...
            // kmeans++ ensures all the clusters has any feature associated with
            // them
...
void TemplatedVocabulary<TDescriptor, F>::initiateClustersKMpp(
    ...
    while ((int)clusters.size() < m_k) {
        ...
        // 3.
        double dist_sum =
            std::accumulate(min_dists.begin(), min_dists.end(), 0.0);
...
void TemplatedVocabulary<TDescriptor, F>::setNodeWeights(
    ...
    if (m_weighting == TF || m_weighting == BINARY) {
        ...
        typename std::vector<vector<TDescriptor> >::const_iterator mit;
        typename std::vector<TDescriptor>::const_iterator fit;

        for (mit = training_features.begin(); mit != training_features.end();
             ++mit) {
...
template <class TDescriptor, class F>
void TemplatedVocabulary<TDescriptor, F>::transform(
    const std::vector<TDescriptor>& features, BowVector& v, FeatureVector& fv,
    int levelsup) const {
    v.clear();
    fv.clear();
...
template <class TDescriptor, class F>
NodeId TemplatedVocabulary<TDescriptor, F>::getParentNode(WordId wid,
                                                          int levelsup) const {
...
template <class TDescriptor, class F>
void TemplatedVocabulary<TDescriptor, F>::save(
    const std::string& filename) const {
