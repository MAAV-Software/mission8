...
void ZcmLoop::run()
{
	...
	while (RUNNING)
	{
		...
		while(STATE_handler.ready()) {
			...
			std::shared_ptr<GaussianData> STATE_attitude = std::dynamic_pointer_cast<GaussianData>(dict_->dict["STATE_attitude"]);
			STATE_attitude->addData(std::move(convertQuaternion(time, STATE_handler.msg().attitude)), covariance.block<3,3>(0,0));
