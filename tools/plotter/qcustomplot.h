...
enum AntialiasedElement
{
    ...
    ,
    aeOther = 0x8000  ///< <tt>0x8000</tt> Other elements that don't fit into any of the existing
                      ///< categories
    ,
...
enum Interaction
{
    ...
    iMultiSelect = 0x004  ///< <tt>0x004</tt> The user can select multiple objects by holding the
                          ///< modifier set by \ref QCustomPlot::setMultiSelectModifier while
                          ///< clicking
    ,
...
enum SelectionRectMode
{
    ...
    srmZoom  ///< When dragging the mouse, a selection rect becomes active. Upon releasing, the axes
             ///< that are currently set as range zoom axes (\ref QCPAxisRect::setRangeZoomAxes)
             ///< will have their ranges zoomed accordingly.
    ,
...
extern const QMetaObject staticMetaObject;  // in moc-run we create a static meta object for QCP
                                            ...
                                            // QCP::staticMetaObject in normal operation as
                                            // namespace
...
public:
    /*!
      ...
      features/fixes/workarounds, depending on whether they are wanted on the respective output
      device.
    */
    enum PainterMode
    {
        ...
        ,
        pmNoCaching = 0x02  ///< <tt>0x02</tt> Mode for all sorts of exports (e.g. PNG, PDF,...).
                            ///< For example, this prevents using cached pixmap labels
        ,
...
public:
    explicit QCPPaintBufferGlFbo(const QSize &size, double devicePixelRatio,
        QWeakPointer<QOpenGLContext> glContext, QWeakPointer<QOpenGLPaintDevice> glPaintDevice);
...
protected:
    ...
    virtual void selectEvent(
        QMouseEvent *event, bool additive, const QVariant &details, bool *selectionStateChanged);
    ...
    void applyAntialiasingHint(
        QCPPainter *painter, bool localAntialiased, QCP::AntialiasedElement overrideElement) const;
...
public:
    ...
    QCPDataSelection &operator-=(const QCPDataSelection &other);
    QCPDataSelection &operator-=(const QCPDataRange &other);
    friend inline const QCPDataSelection operator+(
        const QCPDataSelection &a, const QCPDataSelection &b);
    friend inline const QCPDataSelection operator+(
        const QCPDataRange &a, const QCPDataSelection &b);
    friend inline const QCPDataSelection operator+(
        const QCPDataSelection &a, const QCPDataRange &b);
...
public:
    ...
    */
    ...
    virtual double selectTest(
        const QPointF &pos, bool onlySelectable, QVariant *details = 0) const Q_DECL_OVERRIDE;
...
public:
    ...
    */
    enum EndingStyle
    {
        ...
        ,
        esHalfBar  ///< A bar perpendicular to the line, pointing out to only one side (to which
                   ///< side can be changed with \ref setInverted)
        ,
...
public:
    ...
    */
    enum TickStepStrategy
    {
        ...
        ,
        tssMeetTickCount  ///< Less readable tick steps are allowed which in turn facilitates
                          ///< getting closer to the requested tick count
    };
...
protected:
    ...
    virtual QString getTickLabel(
        double tick, const QLocale &locale, QChar formatChar, int precision);
    virtual QVector<double> createTickVector(double tickStep, const QCPRange &range);
    virtual QVector<double> createSubTickVector(int subTickCount, const QVector<double> &ticks);
    virtual QVector<QString> createLabelVector(
        const QVector<double> &ticks, const QLocale &locale, QChar formatChar, int precision);
...
protected:
    ...
    virtual QString getTickLabel(
        double tick, const QLocale &locale, QChar formatChar, int precision) Q_DECL_OVERRIDE;
...
public:
    ...
    */
    enum ScaleStrategy
    {
        ssNone  ///< Modifications are not allowed, the specified tick step is absolutely fixed.
                ///< This might cause a high tick density and overlapping labels if the axis range
                ///< is zoomed out.
        ,
...
public:
    ...
    */
    enum FractionStyle
    {
        fsFloatingPoint  ///< Fractions are displayed as regular decimal floating point numbers,
                         ///< e.g. "0.25" or "0.125".
        ,
...
protected:
    ...
    virtual void drawTickLabel(
        QCPPainter *painter, double x, double y, const TickLabelData &labelData) const;
    virtual TickLabelData getTickLabelData(const QFont &font, const QString &text) const;
    virtual QPointF getTickLabelDrawOffset(const TickLabelData &labelData) const;
    virtual void getMaxTickLabelSize(
...
public:
    ...
    */
    enum ScatterShape
    {
        ...
        ,
        ssTriangleInverted  ///< \enumimage{ssTriangleInverted.png} an equilateral triangle,
                            ///< standing on corner
        ,
        ...
        ssPixmap  ///< a custom pixmap specified by \ref setPixmap, centered on the data point
                  ///< coordinates
        ,
        ssCustom  ///< custom painter operations are performed per scatter (As QPainterPath, see
                  ///< \ref setCustomPath)
    };
...
public:
    ...
    void limitIteratorsToDataRange(
        const_iterator &begin, const_iterator &end, const QCPDataRange &dataRange) const;
...
{
    if (isEmpty() ||
        !qcpLessThanSortKey<DataType>(data,
            *(constEnd() -
...
void QCPDataContainer<DataType>::removeBefore(double sortKey)
{
    ...
    mPreallocSize += itEnd - it;  // don't actually delete, just add it to the preallocated block
...
{
    QCPDataContainer::iterator it = std::lower_bound(
        begin(), end(), DataType::fromSortKey(sortKey), qcpLessThanSortKey<DataType>);
...
QCPRange QCPDataContainer<DataType>::valueRange(
    bool &foundRange, QCP::SignDomain signDomain, const QCPRange &inKeyRange)
{
    if (isEmpty())
    {
        foundRange = false;
...
void QCPDataContainer<DataType>::performAutoSqueeze()
{
    ...
    else if (totalAlloc > 1000)  // below 10 MiB raw data be generous with preallocated memory,
                                 // below 1k points don't even bother
    {
...
public:
    ...
    virtual double selectTest(const QPointF &pos, bool onlySelectable,
        QVariant *details = 0) const Q_DECL_OVERRIDE = 0;  // actually introduced in QCPLayerable as
                                                           // non-pure, but we want to force
...
public:
    ...
    // reimplemented virtual methods:
    virtual double selectTest(
        const QPointF &pos, bool onlySelectable, QVariant *details = 0) const Q_DECL_OVERRIDE = 0;
...
public:
    ...
    */
    ...
    void setBackground(
        const QPixmap &pm, bool scaled, Qt::AspectRatioMode mode = Qt::KeepAspectRatioByExpanding);
...
protected:
    ...
    QCPLayerable *layerableAt(
        const QPointF &pos, bool onlySelectable, QVariant *selectionDetails = 0) const;
    QList<QCPLayerable *> layerableListAt(
        const QPointF &pos, bool onlySelectable, QList<QVariant> *selectionDetails = 0) const;
    ...
    const QRectF &rect, bool onlySelectable) const
{
    ...
    if (currentSegmentBegin != -1)
        result.addDataRange(
            QCPDataRange(currentSegmentBegin, end - mDataContainer->constBegin()), false);
    ...
    */
    enum GradientPreset
    {
        gpGrayscale  ///< Continuous lightness from black to white (suited for non-biased data
                     ///< representation)
        ,
        gpHot  ///< Continuous lightness from black over firey colors to white (suited for
               ///< non-biased data representation)
        ,
        gpCold  ///< Continuous lightness from black over icey colors to white (suited for
                ///< non-biased data representation)
        ,
        ...
        ,
        gpSpectrum  ///< An approximation of the visible light spectrum (creates banding illusion
...
public:
    ...
    QCPTextElement(
        QCustomPlot *parentPlot, const QString &text, const QString &fontFamily, double pointSize);
    ...
    */
    enum LineStyle
    {
        ...
        lsStepLeft  ///< line is drawn as steps where the step height is the value of the left data
                    ///< point
        ,
        lsStepRight  ///< line is drawn as steps where the step height is the value of the right
                     ///< data point
        ,
    ...
    void setData(
        const QVector<double> &keys, const QVector<double> &values, bool alreadySorted = false);
...
protected:
    ...
    virtual void drawScatterPlot(
        QCPPainter *painter, const QVector<QPointF> &scatters, const QCPScatterStyle &style) const;
    ...
    void getVisibleDataBounds(QCPGraphDataContainer::const_iterator &begin,
        QCPGraphDataContainer::const_iterator &end, const QCPDataRange &rangeRestriction) const;
    ...
    bool segmentsIntersect(
        double aLower, double aUpper, double bLower, double bUpper, int &bPrecedence) const;
    ...
    double pointDistance(
        const QPointF &pixelPoint, QCPGraphDataContainer::const_iterator &closestData) const;
...
protected:
    ...
    virtual void drawScatterPlot(
        QCPPainter *painter, const QVector<QPointF> &points, const QCPScatterStyle &style) const;

    // non-virtual methods:
    void getCurveLines(
        QVector<QPointF> *lines, const QCPDataRange &dataRange, double penWidth) const;
    void getScatters(
        QVector<QPointF> *scatters, const QCPDataRange &dataRange, double scatterWidth) const;
    ...
    QVector<QPointF> getOptimizedCornerPoints(int prevRegion, int currentRegion, double prevKey,
        double prevValue, double key, double value, double keyMin, double valueMax, double keyMax,
        double valueMin) const;
    bool mayTraverse(int prevRegion, int currentRegion) const;
    bool getTraverse(double prevKey, double prevValue, double key, double value, double keyMin,
        double valueMax, double keyMax, double valueMin, QPointF &crossA, QPointF &crossB) const;
    ...
    double pointDistance(
        const QPointF &pixelPoint, QCPCurveDataContainer::const_iterator &closestData) const;
...
public:
    ...
    QCPStatisticalBoxData(double key, double minimum, double lowerQuartile, double median,
        double upperQuartile, double maximum, const QVector<double> &outliers = QVector<double>());
...
public:
    QCPColorMapData(
        int keySize, int valueSize, const QCPRange &keyRange, const QCPRange &valueRange);
    ~QCPColorMapData();
...
public:
    ...
    */
    ...
    void setData(const QVector<double> &keys, const QVector<double> &open,
        const QVector<double> &high, const QVector<double> &low, const QVector<double> &close,
        bool alreadySorted = false);
...
protected:
    ...
    double pointDistance(
        const QPointF &pixelPoint, QCPErrorBarsDataContainer::const_iterator &closestData) const;
    // helpers:
    void getDataSegments(
        QList<QCPDataRange> &selectedSegments, QList<QCPDataRange> &unselectedSegments) const;
...
protected:
    ...
    // non-virtual methods:
    QLineF getRectClippedStraightLine(
        const QCPVector2D &point1, const QCPVector2D &vec, const QRect &rect) const;
    QPen mainPen() const;
};
...
protected:
    ...
    // non-virtual methods:
    QLineF getRectClippedLine(
        const QCPVector2D &start, const QCPVector2D &end, const QRect &rect) const;
    QPen mainPen() const;
};
...
protected:
    ...
    // non-virtual methods:
    QPointF getTextDrawPoint(
        const QPointF &pos, const QRectF &rect, Qt::Alignment positionAlignment) const;
    QFont mainFont() const;
...
protected:
    ...
    void updateScaledPixmap(
        QRect finalRect = QRect(), bool flipHorz = false, bool flipVert = false);
...
public:
    ...
    */
    enum BracketStyle
    {
        ...
        ,
        bsCalligraphic  ///< A curly brace with varying stroke width giving a calligraphic
                        ///< impression
    };
    Q_ENUMS(BracketStyle)
