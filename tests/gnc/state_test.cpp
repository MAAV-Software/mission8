...
BOOST_AUTO_TEST_CASE(StateSimpleMeanTest)
{
	...
	std::vector<double> weights(num_states, 1.0 / static_cast<double>(num_states));
...
BOOST_AUTO_TEST_CASE(StateQuaternionMeanTest)
{
	...
	BOOST_REQUIRE(diff(mean.attitude().unit_quaternion(), Eigen::Quaterniond::Identity()) < tol);

	states[0].attitude() = Sophus::SO3d(Eigen::Quaterniond::Identity());
	states[1].attitude() = Sophus::SO3d(Eigen::Quaterniond(0.707106781, 0.707106781, 0, 0));

	mean = KalmanState::mean(states, weights);
	BOOST_REQUIRE(diff(mean.attitude().unit_quaternion(),
					   Eigen::Quaterniond(0.923879533, 0.382683432, 0, 0)) < tol);

	states[0].attitude() = Sophus::SO3d(Eigen::Quaterniond::Identity());
	states[1].attitude() = Sophus::SO3d(Eigen::Quaterniond(0.707106781, 0, 0, 0.707106781));

	mean = KalmanState::mean(states, weights);
	BOOST_REQUIRE(diff(mean.attitude().unit_quaternion(),
					   Eigen::Quaterniond(0.923879533, 0, 0, 0.382683432)) < tol);
}
...
BOOST_AUTO_TEST_CASE(StateWieghtedMeanTest)
{
	...
	mean = KalmanState::mean(states, weights);
	diff = mean.position() - Eigen::Vector3d(0.3 + 0.6 * 2, 0.3 + 0.6 * 2, 0.3 + 0.6 * 2);
	BOOST_REQUIRE_LE(diff.norm(), tol);
}
