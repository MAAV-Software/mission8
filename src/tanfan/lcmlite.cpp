...
static void deliver_packet(lcmlite_t *lcm, const char *channel, const void *buf, int buf_len)
{
    ...
    for (lcmlite_subscription_t *sub = lcm->first_subscription; sub != NULL; sub = sub->next)
    {
        ...
        if (good) sub->callback(lcm, channel, buf, buf_len, sub->user);
    }
}
...
int lcmlite_receive_packet(lcmlite_t *lcm, const void *_buf, int buf_len, uint64_t from_addr)
{
    ...
    if (magic == MAGIC_LCM2)
    {
        ...
        {
            // malformed packet.
            if (buf_pos >= buf_len || channel_len >= LCM_MAX_CHANNEL_LENGTH) return -2;
    ...
    {
        if (LCM3_NUM_BUFFERS == 0) return -3;
        ...
        if (msg_size > LCM3_MAX_PACKET_SIZE) return -4;

        if (fragments_in_msg > LCM3_MAX_FRAGMENTS) return -5;

        if (fragment_id >= fragments_in_msg || fragment_offset + payload_len > msg_size) return -6;
        ...
        if (fragment_id == 0)
        {
            ...
            while (buf[buf_pos] != 0)
            {
                if (buf_pos >= buf_len || channel_len >= LCM_MAX_CHANNEL_LENGTH) return -8;
...
int lcmlite_publish(lcmlite_t *lcm, const char *channel, const void *_buf, size_t buf_len)
{
    ...
    else
    {
        ...
        while (fragment_offset < buf_len)
        {
            ...
            if (this_fragment_size > max_fragment_size) this_fragment_size = max_fragment_size;
