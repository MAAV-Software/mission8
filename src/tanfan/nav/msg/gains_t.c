...
int64_t __gains_t_hash_recursive(const __lcm_hash_ptr *p)
{
    ...
    for (fp = p; fp != NULL; fp = fp->parent)
        if (fp->v == __gains_t_get_hash) return 0;
    ...
    int64_t hash = (int64_t)0xd0500d89c9fa09bcLL + __float_hash_recursive(&cp) +
...
int gains_t_encoded_size(const gains_t *p) { return 8 + __gains_t_encoded_array_size(p, 1); }
...
int gains_t_decode_cleanup(gains_t *p) { return __gains_t_decode_array_cleanup(p, 1); }
