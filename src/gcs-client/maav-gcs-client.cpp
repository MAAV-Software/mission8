...
public:
	...
	template<typename T>
	void forward_message(const zcm::ReceiveBuffer*,
...
void toVehicle(ZCM& vehicle_zcm, ZCM& gcs_zcm)
{
	...
	gcs_zcm.subscribe(NAV_RUNSTATE_CMD,
		&MessageForwarder::forward_message<nav_runstate_t>, &forwarder);
	gcs_zcm.subscribe(PLANNER_CMD,
	...
	gcs_zcm.subscribe(IDLE_CHANNEL,
		&MessageForwarder::forward_message<idle_t>, &forwarder);
	gcs_zcm.subscribe(START_VISION,
		&MessageForwarder::forward_message<nav_runstate_t>, &forwarder);
...
void toGCS(ZCM& vehicle_zcm, ZCM& gcs_zcm)
{
	...
	vehicle_zcm.subscribe(NAV_RUNSTATE_STAT,
		&MessageForwarder::forward_message<nav_runstate_t>, &forwarder);
	vehicle_zcm.subscribe(CTRL_HEARTBEAT_CHANNEL,
		&MessageForwarder::forward_message<waypoint_t>, &forwarder);
	vehicle_zcm.subscribe(CAMERA_DISC_STAT,
		&MessageForwarder::forward_message<camera_disc_t>, &forwarder);
	vehicle_zcm.subscribe(VISION_STAT,
		&MessageForwarder::forward_message<nav_runstate_t>, &forwarder);
	vehicle_zcm.subscribe(PLANNER_STAT,
		&MessageForwarder::forward_message<waypoint_t>, &forwarder);
	vehicle_zcm.subscribe(STATE_CHANNEL,
		&MessageForwarder::forward_message<state_t>, &forwarder);
	vehicle_zcm.subscribe(OBST_HEARTBEAT_CHANNEL,
		&MessageForwarder::forward_message<nav_runstate_t>, &forwarder);

	MAAV_DEBUG("Channels to GCS:\n\t%s\n\t%s\n\t%s\n\t%s\n\t%s\n\t%s",
		NAV_RUNSTATE_STAT,
		CTRL_HEARTBEAT_CHANNEL,
		CAMERA_DISC_STAT,
		VISION_STAT,
		STATE_CHANNEL,
...
int main(int argc, char** argv)
{
	...
	gopt.addString('c', "config", "../../config/gcs-client-config.yaml",
	...
	if (not vehicle_zcm.good())
		throw runtime_error{"Vehicle listener ZCM initialization failed!"};
	if (not gcs_zcm.good())
		throw runtime_error{"GCS listener ZCM initialization failed!"};
