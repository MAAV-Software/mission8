...
void toVehicle(ZCM& vehicle_zcm, ZCM& gcs_zcm)
{
	...
	gcs_zcm.subscribe(NAV_RUNSTATE_CMD, &MessageForwarder::forward_message<nav_runstate_t>,
					  &forwarder);
	gcs_zcm.subscribe(PLANNER_CMD, &MessageForwarder::forward_message<waypoint_t>, &forwarder);
	gcs_zcm.subscribe(CAMERA_DISC_CMD, &MessageForwarder::forward_message<camera_disc_t>,
					  &forwarder);
	gcs_zcm.subscribe(CTRL_PARAMS_CHANNEL, &MessageForwarder::forward_message<ctrl_params_t>,
					  &forwarder);
...
void toGCS(ZCM& vehicle_zcm, ZCM& gcs_zcm)
{
	...
	vehicle_zcm.subscribe(NAV_RUNSTATE_STAT, &MessageForwarder::forward_message<nav_runstate_t>,
						  &forwarder);
	vehicle_zcm.subscribe(CTRL_HEARTBEAT_CHANNEL, &MessageForwarder::forward_message<waypoint_t>,
						  &forwarder);
	vehicle_zcm.subscribe(CAMERA_DISC_STAT, &MessageForwarder::forward_message<camera_disc_t>,
						  &forwarder);
	vehicle_zcm.subscribe(VISION_STAT, &MessageForwarder::forward_message<nav_runstate_t>,
						  &forwarder);
