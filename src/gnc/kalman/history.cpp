...
History::Iterator History::find_snapshot(uint64_t time, uint64_t tolerance) {
    ...
    for (auto r_iter = std::next(end); r_iter != _history.rend();
         std::advance(r_iter, 1)) {
        ...
        if (time > snapshot_time) {
            ...
            if (time - prev_t <= tolerance) {
                ...
                ImuMeasurement interp_imu =
                    interpolate_imu(*(prev_iter->measurement.imu),
                                    *(next_iter->measurement.imu), time);
                Measurement interp_measurement;
                interp_measurement.imu =
                    std::make_shared<ImuMeasurement>(interp_imu);
...
ImuMeasurement History::interpolate_imu(const ImuMeasurement &prev,
                                        const ImuMeasurement &next,
    ...
    interpolated.angular_rates +=
        weight * (next.angular_rates - prev.angular_rates);
    interpolated.acceleration +=
        weight * (next.acceleration - prev.acceleration);
    interpolated.magnetometer +=
        weight * (next.magnetometer - prev.magnetometer);

    return interpolated;
}
History::Iterator History::set_last_modified(
    const History::Iterator last_modified,
