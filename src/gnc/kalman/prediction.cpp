...
{
    const KalmanState& prev_state = state;
    ...
    next_state.gyro_bias() = prev_state.gyro_bias();
    next_state.accel_bias() = prev_state.accel_bias();

    const Eigen::Vector3d& w_prev = prev_imu.angular_rates - prev_state.gyro_bias();
    const Eigen::Vector3d& w_next = next_imu.angular_rates - next_state.gyro_bias();
    ...
    const Eigen::Vector3d& a_prev =
        prev_state.attitude() * (prev_imu.acceleration - prev_state.accel_bias()) - gravity;
    const Eigen::Vector3d& a_next =
        next_state.attitude() * (next_imu.acceleration - next_state.accel_bias()) - gravity;
