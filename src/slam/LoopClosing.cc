...
bool LoopClosing::DetectLoop() {
    ...
    for (size_t i = 0, iend = vpCandidateKFs.size(); i < iend; i++) {
        ...
        for (size_t iG = 0, iendG = mvConsistentGroups.size(); iG < iendG;
             iG++) {
            ...
            if (bConsistent) {
                ...
                if (!vbConsistentGroup[iG]) {
                    ConsistentGroup cg =
                        make_pair(spCandidateGroup, nCurrentConsistency);
                    vCurrentConsistentGroups.push_back(cg);
                    vbConsistentGroup[iG] = true;  // this avoid to include the
                                                   // same group more than once
                }
                if (nCurrentConsistency >= mnCovisibilityConsistencyTh &&
                    !bEnoughConsistent) {
                    mvpEnoughConsistentCandidates.push_back(pCandidateKF);
                    bEnoughConsistent = true;  // this avoid to insert the same
...
bool LoopClosing::ComputeSim3() {
    ...
    for (int i = 0; i < nInitialCandidates; i++) {
        ...
        if (nmatches < 20) {
            ...
            Sim3Solver* pSolver = new Sim3Solver(
    ...
    while (nCandidates > 0 && !bMatch) {
        for (int i = 0; i < nInitialCandidates; i++) {
            ...
            // If RANSAC returns a Sim3, perform a guided matching and optimize
            // with all correspondences
            if (!Scm.empty()) {
                ...
                matcher.SearchBySim3(mpCurrentKF, pKF, vpMapPointMatches, s, R,
                                     t, 7.5);

                g2o::Sim3 gScm(Converter::toMatrix3d(R),
                               Converter::toVector3d(t), s);
...
void LoopClosing::CorrectLoop() {
...

    {
        ...
        for (KeyFrameAndPose::iterator mit = CorrectedSim3.begin(),
            ...
            Eigen::Matrix3d eigR =
                g2oCorrectedSiw.rotation().toRotationMatrix();
        ...
        if (!mbStopGBA) {
            ...
            for (size_t i = 0; i < vpMPs.size(); i++) {
                ...
                if (pMP->mnBAGlobalForKF == nLoopKF) {
                    ...
                    cv::Mat Rcw =
                        pRefKF->mTcwBefGBA.rowRange(0, 3).colRange(0, 3);
