...
void zcm_thread(ZCMAccumulator* zcm_acc,zcm::ZCM *zcmPtr)
{
	...
	while (run)
	{
		while (!zcm_acc->ready()) {zcm_acc->cv.wait(lck);}
	...
	condition_variable *imgFeedWaitCvPtr, atomic<bool> *imgFeedReadyPtr)
{
	...
	for (int i {0}; i < numAvailableCams; ++i)
	{
		if (config["vision_driver"]["type"].as<int>() != 2)
		{
			cap[i] = new VideoInterface(config["vision_driver"]
				["type"].as<int>());
		}
		...
		{
			cap[i]->open(config["vision_driver"]["filenames"][i]
				.as<string>());
		}
		else
		{
	...
	if (useGCS)
	{
		...
		if (zcm.good())
			zcm.publish(maav::CAMERA_DISC_STAT,&cameraDiscMsg);
	...
	for (bool ready {false}; !ready && useGCS && identifyCams; )
	{
		...
		msg.numCameras = camIdentifier.identify(cap,activeCams,*img_feed,useGCS,
			zcmPtr,&identifyHandler);
		if (msg.numCameras == 5)
		{
			ready = true;
		}
		...
		if (zcm.good())
			zcm.publish(maav::CAMERA_DISC_STAT,&msg);
	}
	if (identifyCams && !useGCS)
	{
		cout << "Identifying cameras\n";
		CameraIdentifier camIdentifier(config["vision_driver"]["cameraIdThresh"].as<int>());
		cout << camIdentifier.identify(cap,activeCams,*img_feed,useGCS, zcmPtr,
			&identifyHandler) << " cameras found\n";
	}
	for (int i {0}; i < 5; ++i)
	{
...
// Reworked to new design: COMPLETE
void RoombaDetection(ImageFeed* img_feed, bool is_bottom, int camera,
	ZCMAccumulator* zcm_acc,zcm::ZCM *zcmPtr,VehicleStateRepo* stateHandlerPtr)
{
	...
	while (run)
	{
		...
		try
		{
			...
			#endif
			{
				...
				detectObject.combineDetect(*framePtr,cloud,
					fakeGroundPlane,roombas,
					camera,timestamp);
			}
#ifdef BENCHMARKING
			...
			if (!detectObject.locations.size())
				throw camera;
...
int main(int argc, char** argv)
{
	...
	{
		dataCoord = std::make_shared<DataCoordinator>(
			config["vision_driver"]["reader_delay"].as<int>(),
		...
		for (int i {0}; i < 5; ++i)
		{
			img_feeds.push_back(new ImageFeed(
				config["vision_driver"]["writeSize"].as<int>(),
	...
	while (!imgFeedReady) { imgFeedWaitCv.wait(imgFeedWaitLck); }
        // Create Roomba Threads
	if (config["vision_driver"]["run_roombas"].as<bool>())
	{
		for (int i {0}; i < 5; ++i)
		{
			roomba_threads.emplace_back(std::thread(RoombaDetection,
				img_feeds[i], !(bool)i, i, &zcm_acc, &zcm,&stateHandler));
			std::this_thread::sleep_for(std::chrono::milliseconds(3));
		}
	}
	// Create Line Threads
	if (config["vision_driver"]["run_lines"].as<bool>())
	{
		for (int i {0}; i < 5; ++i)
		{
			line_detection_threads.emplace_back(std::thread(LineDetection,
				img_feeds[i], i, &zcm_acc, &zcm,&stateHandler));
			std::this_thread::sleep_for(std::chrono::milliseconds(3));
		}
	}
	...
	if (config["vision_driver"]["is_test"].as<bool>())
		std::thread(testVersionTerminator).detach();
