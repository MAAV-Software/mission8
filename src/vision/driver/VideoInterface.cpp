...
class VideoInterface::HandlerFrame
{
	...
	void handleMessage(const zcm::ReceiveBuffer *rbuf,
		const std::string &chan, const cam_capture_t *msg)
	{
		frameData = *msg;
	}
};
...
class VideoInterface::cvVideoCapture : public VideoInterface::Impl
{
	public:
		...
		virtual bool retrieve(cv::Mat &image, int channel)
			{ return cap.retrieve(image,channel); }
...
class VideoInterface::RGBDGetterInterface : public VideoInterface::Impl
{
	public:
		...
		virtual bool retrieve(std::vector<Point> & cloud, int channel)
		{
			return read(cloud);
		}
...
{
	public:
		PhysSimInterface(VisionSimZCMData* zcmDataIn)
			: opened {false}, isOwner {false}
			{
				zcmData = *zcmDataIn;
		...
		virtual void init(VisionSimZCMData *&zcmDataOut)
		{
			...
			zcmData.zcmImage->subscribe("CAM",&HandlerFrame::
				handleMessage,zcmData.handler);
...
void VideoInterface::init(VisionSimZCMData *&zcmDataOut)
{
	return pimpl->init(zcmDataOut);
}

bool VideoInterface::open(int index)
{
	return pimpl->open(index);
}

bool VideoInterface::open(std::string file)
{
	return pimpl->open(file);
}

bool VideoInterface::isOpened()
{
...
bool VideoInterface::read(cv::Mat &image)
{
	return pimpl->read(image);
}
...
bool VideoInterface::read(pcl::PointCloud<pcl::PointXYZ> &cloud)
{
	return pimpl->read(cloud);
}
...
bool VideoInterface::read(std::vector<pf::Point3f> & cloud)
{
	return pimpl->read(cloud);
}

double VideoInterface::get(int propID)
{
	return pimpl->get(propID);
}

bool VideoInterface::set(int propID, double value)
{
	return pimpl->set(propID,value);
}
...
rs::context * VideoInterface::getContext() {
	return pimpl->getContext();
}

void VideoInterface::setContext(rs::context * context) {
