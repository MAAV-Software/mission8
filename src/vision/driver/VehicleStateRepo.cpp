...
void VehicleStateRepo::handleMessage(const zcm::ReceiveBuffer *rbuf,
...
bool VehicleStateRepo::getState(std::vector<double> &input, int64_t timestamp)
{
	...
	while (true)
	{
		...
		else
		{
			...
			if (getTimeDiff(index + 1, timestamp, otherTimeDiff) && otherTimeDiff <
				timeDiffMid)
			{
			...
			else if (getTimeDiff(index - 1, timestamp, otherTimeDiff) && otherTimeDiff <
				timeDiffMid)
			{
				double *statePtr = stateHistory.states[index - 1].state;
				input.assign(statePtr,statePtr + 13);
