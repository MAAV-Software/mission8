...
class TimeInterface::SimTimeInterface : public TimeInterface::Impl
{
   private:
	...
	std::map<int64_t, std::pair<std::condition_variable *, std::mutex *>> conditionVariables;
   ...
   public:
	SimTimeInterface();
	static void zcmThread(HandlerSimTime *handler, SimTimeInterface *simTimer);
	virtual void init();
	virtual void sleep(int duration);
	virtual void getTime(int64_t &time);
	void release();
...
TimeInterface::SimTimeInterface::SimTimeInterface() { releaseBool = false; }
void TimeInterface::SimTimeInterface::zcmThread(HandlerSimTime *handler, SimTimeInterface *simTimer)
{
	zcm::ZCM zcm{"ipc"};
...
void TimeInterface::SimTimeInterface::sleep(int duration)
{
	...
	std::condition_variable *cvPtr;
	{
		...
		conditionVariables.insert(
			std::pair<int64_t, std::pair<std::condition_variable *, std::mutex *>>(
				inTime, std::pair<std::condition_variable *, std::mutex *>(
...
void TimeInterface::init() { pimpl->init(); }
void TimeInterface::sleep(int duration) { pimpl->sleep(duration); }
void TimeInterface::getTime(int64_t &time) { pimpl->getTime(time); }
