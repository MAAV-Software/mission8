...
void randomTest(std::ofstream &fout, std::ofstream &foutError)
{
	...
	if (coefs.size() == 0)
	{
		cout << "Plane fitting failed under these conditions: \n";
		cout << "inlier distance: " << inlierDistance << ", noise: " << noise << '\n';
		cout << "x = " << x << ", y = " << y << ", z = " << z << ", d = " << d << '\n';
	}
	else
	{
		coefficients_t coefStruct = {coefs(0, 0), coefs(1, 0), coefs(2, 0), coefs(3, 0)};
		coefficients_t groundCoefs = {x, y, z, d};
		if (checkCoefficients(groundCoefs, coefStruct))
		{
			return;
		}
		else
		{
			cout << "Plane fitting has reached insufficient results here:\n";
			cout << "inlier distance: " << inlierDistance << ", noise: " << noise << '\n';
			cout << "x = " << x << ", y = " << y << ", z = " << z << ", d = " << d << '\n';
			cout << "Resulting fit plane:\n";
			cout << "x = " << coefs(0, 0) << ", y = " << coefs(1, 0) << ", z = " << coefs(2, 0);
			cout << ", d = " << coefs(3, 0) << std::endl;
		}
	}
