...
int main(int argc, char** argv)
{
	...
	while (currentlyOn)
	{
		...
		if (writeCombined)
		{
			threads.emplace_back(storeCombined, colorImage, depthImage, counter,
				camNumGlobal);
		}
		...
		if (writeCloud)
		{
			...
			for (int dy {0}; dy < depthIntrinsics.height; ++dy)
			{
				for (int dx {0}; dx < depthIntrinsics.width; ++dx)
				{
					...
					uint16_t depthValue =
						depthImage[dy * depthIntrinsics.width + dx];
					...
					if (cy > color_intrin.height || cx > color_intrin.width ||
						cy < 0 || cx < 0)
					{
						xyzrgbaPoint.r = 255;
						xyzrgbaPoint.g = 255;
					...
					cloud.push_back(pcl::PointXYZ(depthPoint.x, depthPoint.y,
						depthPoint.z));
				}
			}
			// Store point cloud
