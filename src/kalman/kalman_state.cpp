...
KalmanState::CovarianceMatrix KalmanState::cov(
    const KalmanState& mean, const std::vector<KalmanState>& sigma_points,
    const std::vector<double>& weights) {
    std::vector<ErrorStateVector> points;
    for (const KalmanState& state : sigma_points) {
        ErrorStateVector e_state;
        Eigen::Vector3d q_err =
            (mean.attitude().inverse() * state.attitude()).log();
...
KalmanState KalmanState::compute_gaussian(
    const std::vector<KalmanState>& points,
...
KalmanState& KalmanState::operator+=(
    const KalmanState::ErrorStateVector& e_state) {
