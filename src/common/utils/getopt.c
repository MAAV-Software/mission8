...
getopt_t *getopt_create(void)
{
	...
	gopt->lopts =
		zhash_create(sizeof(char *), sizeof(getopt_option_t *), zhash_str_hash, zhash_str_equals);
	gopt->sopts =
		zhash_create(sizeof(char *), sizeof(getopt_option_t *), zhash_str_hash, zhash_str_equals);
...
int getopt_parse(getopt_t *gopt, int argc, char *argv[], int showErrors)
{
	...
	while (i < zarray_size(toks))
	{
		...
		if (0 == strncmp(tok, "--", 2))
		{
			...
			{
				okay = 0;
				if (showErrors) printf("Unknown option --%s\n", optname);
				i++;
				continue;
			}
			...
			if (goo->type == GOO_STRING_TYPE)
			{
				...
				okay = 0;
				if (showErrors) printf("Option %s requires a string argument.\n", optname);
			}
		}
		...
		if (0 == strncmp(tok, "-", 1) && strncmp(tok, "--", 2))
		{
			...
			for (pos = 1; pos < len; pos++)
			{
				...
				if (goo == NULL)
				{
					...
					{
						okay = 0;
						if (showErrors) printf("Unknown option -%c\n", tok[pos]);
						i++;
						continue;
					}
				}
				...
				if (goo->type == GOO_STRING_TYPE)
				{
					...
					okay = 0;
					if (showErrors) printf("Option -%c requires a string argument.\n", tok[pos]);
				}
			}
			i++;
			continue;
...
void getopt_add_bool(getopt_t *gopt, char sopt, const char *lname, int def, const char *help)
{
	...
	if (sopt == '-')
	{  // short name cannot be '-' (no way to reference)
...
void getopt_add_double(getopt_t *gopt, char sopt, const char *lname, const char *def,
					   const char *help)
{
	getopt_add_string(gopt, sopt, lname, def, help);
}
...
int getopt_was_specified(getopt_t *getopt, const char *lname)
{
	...
	if (goo == NULL) return 0;

	return goo->was_specified;
}

const zarray_t *getopt_get_extra_args(getopt_t *gopt) { return gopt->extraargs; }
...
void getopt_do_usage(getopt_t *gopt)
{
	...
	for (unsigned int i = 0; i < zarray_size(gopt->options); i++)
	{
		...
		if (goo->spacer) continue;

		longwidth = max(longwidth, strlen(goo->lname));

		if (goo->type == GOO_STRING_TYPE) valuewidth = max(valuewidth, strlen(goo->svalue));
	}
