...
zhash_t *zhash_copy(zhash_t *orig)
{
    assert(orig != NULL);
...
int zhash_get(const zhash_t *zh, const void *key, void *out_value)
{
    ...
    for (int i = 0; i < bucket->size; i++)
    {
        ...
        if (zh->equals(key, this_key))
        {
            ...
            if (out_value != NULL) memcpy(out_value, this_value, zh->valuesz);
            return 1;
        }
    }
...
int zhash_get_volatile(const zhash_t *zh, const void *key, void *out_value)
{
    ...
    for (int i = 0; i < bucket->size; i++)
    {
        ...
        {
            void *this_value = &bucket->values[zh->valuesz * i];
            if (out_value != NULL) *((void **)out_value) = this_value;
            return 1;
        }
    }
...
static inline int zhash_put_real(zhash_t *zh, struct bucket *buckets, int nbuckets, const void *key,
    const void *value, void *old_key, void *old_value)
{
    assert(zh != NULL);
    assert(buckets != NULL);
    assert(key != NULL);
    ...
    for (int i = 0; i < bucket->size; i++)
    {
        ...
        if (zh->equals(key, this_key))
        {
            ...
            if (old_key) memcpy(old_key, this_key, zh->keysz);

            if (old_value) memcpy(old_value, this_value, zh->valuesz);
...
int zhash_iterator_next_volatile(zhash_iterator_t *zit, void *outkey, void *outvalue)
{
    ...
    while (zit->bucket < zh->nbuckets)
    {
        ...
        if (zit->idx < bucket->size)
        {
            ...
            if (outkey != NULL) *((void **)outkey) = this_key;
            if (outvalue != NULL) *((void **)outvalue) = this_value;
            zit->idx++;
...
int zhash_iterator_next(zhash_iterator_t *zit, void *outkey, void *outvalue)
{
    ...
    if (res)
    {
        if (outkey != NULL) memcpy(outkey, outkeyp, zit->zh->keysz);
        if (outvalue != NULL) memcpy(outvalue, outvaluep, zit->zh->valuesz);
    }
    return res;
}
...
void zhash_map_keys(zhash_t *zh, void (*f)())
{
    ...
    while (zhash_iterator_next_volatile(&itr, &key, &value)) f(key);
}
...
void zhash_map_values(zhash_t *zh, void (*f)())
{
    ...
    void *key, *value;
    while (zhash_iterator_next_volatile(&itr, &key, &value)) f(value);
}
...
zarray_t *zhash_keys(zhash_t *zh)
{
    ...
    while (zhash_iterator_next_volatile(&itr, &key, &value)) zarray_add(za, key);
...
zarray_t *zhash_values(zhash_t *zh)
{
    ...
    while (zhash_iterator_next_volatile(&itr, &key, &value)) zarray_add(za, value);
