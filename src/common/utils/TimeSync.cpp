...
TimeSync::TimeSync(double alpha1_, double alpha2_):
	alpha1(alpha1_), alpha2(alpha2_) {}
...
long TimeSync::reclock(long tivaTimeStamp, long recvTimeStamp)
{
	...
	{
		if (tivaTimeStamp - recvTimeStamp - f(0) >= lastGoodP -
			lastGoodQ - f(tivaTimeStamp - lastGoodP))
		{
