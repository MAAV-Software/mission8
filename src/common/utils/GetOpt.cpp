...
void GetOpt::swap(GetOpt& other) noexcept
{
	std::swap(gopt, other.gopt);
}
...
const char * GetOpt::getString(const char *lname)
{
	return getopt_get_string(gopt, lname);
}

int GetOpt::getInt(const char *lname)
{
	return getopt_get_int(gopt, lname);
}

bool GetOpt::getBool(const char *lname)
{
	return getopt_get_bool(gopt, lname) == 1;
}

double GetOpt::getDouble(const char *lname)
{
	return getopt_get_double(gopt, lname);
}

bool GetOpt::wasSpecified(const char *lname)
{
	return getopt_was_specified(gopt, lname) == 1;
}

const zarray_t * GetOpt::getExtraArgs()
{
	return getopt_get_extra_args(gopt);
}
