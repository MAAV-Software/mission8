...
CommandFrame::CommandFrame(GlibZCM& zcm, const GCSConsts& consts_in) :
					Gtk::Frame("Command"), CONSTS{consts_in},
					...
					xcontroller({0.5, 0.5, 0.5, 0.5}, 0.0)
{
	...
	runstate.signal_update().connect([this](const nav_runstate_t& stat) {
		update_runstate_btn(stat);
	});
	waypt.signal_update().connect([this](const waypoint_t& stat) {
		update_waypt_btns(stat);
	});
...
void CommandFrame::update_waypt()
{
	if(autonomous.get_active()){
		...
		if(chk_pose.get_active() and chk_rate.get_active()) {
			way.mode = 3;
			double poses[] = {xp.get_value(), yp.get_value(),
					  -zp.get_value(), yawp.get_value()};
			double rates[] = {xr.get_value(), yr.get_value(),
					  zr.get_value(), yawr.get_value()};
