...
const string StatusFrame::status_text(Status stat, const char* extra)
{
	switch (stat)
	{
		...
		case Status::quiet:
			return u8"<span foreground=\"yellow\"><tt>\U0001f610 QUIET </tt></span>";
		case Status::down:
			return u8"<span foreground=\"red\"><tt>\U0001f615 DOWN  </tt></span>";
		case Status::calibrated:
			char text[256];
			sprintf(text, u8"<span foreground=\"green\"><tt>\U0001f60e CALIBRATED %s</tt></span>",
					extra);
			return string{text};
...
StatusFrame::StatusFrame(GlibZCM& zcm, const GCSConsts& consts_in)
	: Gtk::Frame("Status"),
	  CONSTS{consts_in},
	  ...
	  controller_info{zcm, CTRL_HEARTBEAT_CHANNEL, CONSTS}
{
	...
	upper_box.add(path_planner), upper_box.add(localization), upper_box.add(controller);
	upper_cbox.set_center_widget(upper_box);
	outer_box.add(upper_cbox);

	lower_box.add(line_det), lower_box.add(roomba_det), lower_box.add(obstacle_det);
