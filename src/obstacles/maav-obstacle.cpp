...
public:
	LidarHandler(shared_ptr<zcm::ZCM> zcmptr_in,
			shared_ptr<ZCMSingleHandler<state_t> > state_ptr_in):
			zcmptr{zcmptr_in},
...
int main (int argc, char** argv)
{
	...
	emptystate.utime =
				static_cast<int64_t>(
						chrono::time_point_cast<chrono::microseconds>(
						chrono::system_clock::now()
					).time_since_epoch().count());

	// Construct a ZCMSingleHandler with an empty state
	shared_ptr<ZCMSingleHandler<state_t> > zs =
		make_shared<ZCMSingleHandler<state_t> >( emptystate );
