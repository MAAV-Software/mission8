...
vector<ObstacleDetector::obstacle_data> ObstacleDetector::detect(const vector<float>& laser_data)
{
	...
	// stop if none exists
	while ((region_start = find_if(region_start, end(laser_data), not_far)) != end(laser_data))
	{
		...
		{
			while (obst_end != region_end and abs(obst_end[0] - obst_end[-1]) < jump_margin)
				++obst_end;
			...
			float mid_dist{(obst_size & 1)
							   ? obst_start[obst_size / 2]
			...
			// calculate the width
			float width{(*obst_start + obst_end[-1]) * sin((obst_size - 1) * step / 2.0)};

			// insert into datas
			datas.push_back({{mid_dist * cos(mid_theta), mid_dist * sin(mid_theta)}, width});
