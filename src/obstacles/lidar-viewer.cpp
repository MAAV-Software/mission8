   ...
   public:
	LidarViewer(bool hold_in)
		: hold(hold_in),
		  width{1000},
		  height{1000},
		  myMat(width, height, CV_8UC3, cv::Scalar(255, 255, 255)),
		  myThread{[this]() {
	...
	void view(const laser_t theLaser, vector<maav::ObstacleDetector::obstacle_data> ob_data)
	{
		...
		{
			cv::circle(myMat, cv::Point(width / 2, height / 2), i * width / max_lidar_range / 2,
					   cv::Scalar(0, 0, 0), 1);
		}
		...
		for (size_t i = 0; i < theLaser.ranges.size(); i++)
		{
			...
			int x = sin(thisRad) * width / 2 * theLaser.ranges[i] / max_lidar_range;
			int y = cos(thisRad) * width / 2 * theLaser.ranges[i] / max_lidar_range;
			// cout << "x = " << x << "\ty = "<< y << endl;

			cv::circle(myMat, cv::Point(width / 2 + x, height / 2 + y), 1, cv::Scalar(255, 0, 0),
					   5);
		}
		for (size_t i = 0; i < ob_data.size(); i++)
		{
			cv::circle(
				myMat,
				cv::Point(width / 2 + ob_data[i].midpoint.x() * width / max_lidar_range / 2,
						  height / 2 + ob_data[i].midpoint.y() * height / max_lidar_range / 2),
				30, cv::Scalar(0, 0, 255), 2);
		}
