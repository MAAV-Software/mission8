...
class StatusFrame : public Gtk::Frame
{
	...
	enum class Status {online, quiet, down, calibrated, uncalibrated};
	...
	public:
		...
		StatusDisplay(GlibZCM& zcm, const char* channel, const char* name_text,
			...
			: GlibZCM::Handler<Message>(zcm, channel), name{name_text},
			cur_status{Status::uncalibrated}, CONSTS{consts_in}
	...
	protected:
		...
		virtual void on_message(const Message&) override
		{
		...
		virtual void update_status(Status new_status)
		{
			...
			if (new_status != Status::down) {
				...
				timer_connection = Glib::signal_timeout().connect_seconds(
					[this]() {
						update_status(
							cur_status == Status::online ?
								Status::quiet : Status::down
						);
						return false;
					},
					cur_status == Status::online ? CONSTS.QUIET_TIMEOUT
								     : CONSTS.DOWN_TIMEOUT
				);
			}
		}
	...
	protected:
		virtual void on_message(const waypoint_t& msg) override
		{
			...
			std::stringstream ss;
			switch (msg.pmode) {
			case 0: mode_str = "Autonomous"; break;
			case 1: mode_str = "Hover"; break;
			case 2: mode_str = "Takeoff"; break;
			case 3: mode_str = "Land"; break;
			case 4: if(msg.mode == 1 || msg.mode == 3) {
				   ss << std::setprecision(3) << "Pose: ("
				   << msg.pose[0] << ", " << msg.pose[1] << ", "
				...
				mode_str = ss.str();
				break;
			case 5: mode_str = "Exit Arena"; break;
			default: mode_str = "No Mission Running";
			}
