...
{
	using namespace std;
	return equal(begin(a.value), end(a.value), begin(b.value))
		and equal(begin(a.rate), end(a.rate), begin(b.rate));
}
...
{
	return a.mode == b.mode and a.pmode == b.pmode
		and std::equal(std::begin(a.pose), std::end(a.pose), std::begin(b.pose))
		and std::equal(std::begin(a.rate), std::end(a.rate), std::begin(b.rate));
}
