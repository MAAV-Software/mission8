...
class RoombaHandler
{
	public:
		...
		RoombaHandler()
		{
			lck = std::unique_lock<std::mutex>(cvMtx);
		}
		void handleMessage(const zcm::ReceiveBuffer *rbuf,
			const std::string &chan, const roomba_list_t *msg)
		{
...
class LineHandler
{
	...
	public:
		...
		void handleMessage(const zcm::ReceiveBuffer *rbuf,
			const std::string &chan, const visual_data_t *msg)
		{
			std::lock_guard<std::mutex> l(mtx);
...
public:
	...
	void insert(std::shared_ptr<cv::Mat> &framePtr, int64_t timestamp,
		int camID, std::shared_ptr<std::shared_mutex> &sharedMtx);
	...
	static void listenerThreadR(DataCoordinator* dataCoord,
		RoombaHandler* roombaHandler,std::atomic<bool> *isRunning,
		zcm::ZCM *zcmPtr);
	// zcm listener thread for visual_data_t messages
	// recieves "Lines" messages and writes data to frame
	static void listenerThreadL(DataCoordinator* dataCoord,
		LineHandler* lineHandler,std::atomic<bool> *isRunning,
		zcm::ZCM *zcmPtr);
