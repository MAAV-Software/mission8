...
public:
    ...
    * 
    * \param    widthInMeters           Width of the grid in meters -- will be rounded up to nearest cell unit
    * \param    heightInMeters          Height of the grid in meters -- will be rounded up to nearest cell unit
    * \param    metersPerCell           Side length of a cell in the grid
    * 
    * \pre  widthInMeters  > 0
    ...
    */
    ...
    Eigen::Vector2f originInGlobalFrame(void) const { return globalOrigin_; }
    ...
    */
    CellOdds cellOdds(int x, int y) const;
    ...
    */
    void setCellOdds(int x, int y, CellOdds logOdds);
    ...
    */
    occupancy_grid_t toZCM(void) const;
    
    /**
    * fromZCM populates the grid using an ZCM message. The current contents of the grid are erased.
    */
    void fromZCM(const occupancy_grid_t& gridMessage);
    ...
    */
	...
	Eigen::Vector2d lin2coord(int i) const { return Eigen::Vector2d(i % width_, i / width_);}
...
private:
    ...
    Eigen::Vector2f globalOrigin_;         ///< Origin of the grid in global coordinates
};
}
}
