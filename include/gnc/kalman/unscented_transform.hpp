...
class UnscentedTransform {
   public:
    ...
    UnscentedTransform(Transform transform, double alpha, double beta,
                       double kappa)
...
template <class TargetSpace>
TargetSpace UnscentedTransform<TargetSpace>::operator()(
    ...
    std::transform(sigma_points.begin(), sigma_points.end(),
                   std::back_inserter(transformed_points), _transformation);

    // Recompute the mean and covariance
    TargetSpace result =
        TargetSpace::compute_gaussian(transformed_points, m_weights, c_weights);
    return result;
}

template <class TargetSpace>
typename UnscentedTransform<TargetSpace>::SigmaPoints UnscentedTransform<
    TargetSpace>::generate_sigma_points(const KalmanState& state) {
    constexpr size_t N = KalmanState::E_DIM;

    Eigen::LLT<KalmanState::CovarianceMatrix> decomp(
        (static_cast<double>(N) + _lambda) * state.covariance());
