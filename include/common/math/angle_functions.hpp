...
inline double wrap_to_pi_2(double angle)
{
    ...
    if (wrapped < -M_PI_2) 		wrapped += M_PI;
    else if (wrapped > M_PI_2) 	wrapped -= M_PI;
...
/**
* angle_diff_abs finds the absolute value of the difference in radians between two angles and ensures that the differences
