...
public:
	...
	std::pair<typename std::deque<RecordPtr<T> >::iterator,
			  typename std::deque<RecordPtr<T> >::iterator> get(int64_t queryTime);
	 ...
	 */
	typename std::deque<RecordPtr<T>>::iterator insert(typename std::deque<RecordPtr<T>>::iterator position,
													   RecordPtr<T> record);
private:
...
template <typename T>
typename std::deque<RecordPtr<T>>::iterator Tracker<T>::insert(typename std::deque<RecordPtr<T>>::iterator position,
															   RecordPtr<T> record)
{
...
template <typename T>
std::pair<typename std::deque<RecordPtr<T> >::iterator,
		  typename std::deque<RecordPtr<T> >::iterator> Tracker<T>::get(int64_t queryTime)
{
	...
	if ((history.size() >= 1) && ((*history.begin())->second == queryTime)) //special case: we need to get the first element
