...
#define DEPTH_PUB_FREQ_HZ 15
#define COLOR_PUB_FREQ_HZ 15
#define IRED1_PUB_FREQ_HZ 15
#define IRED2_PUB_FREQ_HZ 15
...
void RealSensePlugin::Load(physics::ModelPtr _model, sdf::ElementPtr /*_sdf*/) {
    ...
    this->dataPtr->newIred1FrameConn =
        this->dataPtr->ired1Cam->ConnectNewImageFrame(
            std::bind(&RealSensePlugin::OnNewFrame, this,
                      this->dataPtr->ired1Cam, this->dataPtr->ired1Pub));

    this->dataPtr->newIred2FrameConn =
        this->dataPtr->ired2Cam->ConnectNewImageFrame(
            std::bind(&RealSensePlugin::OnNewFrame, this,
                      this->dataPtr->ired2Cam, this->dataPtr->ired2Pub));
...
void RealSensePlugin::OnNewDepthFrame() const {
    ...
    if (this->dataPtr->depthMap.size() != imageSize) {
        try {
            ...
            std::cerr << "RealSensePlugin: depthMap allocation failed: "
                      << e.what() << std::endl;
            return;
        }
    }
